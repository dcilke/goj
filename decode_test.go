...
func TestReadByte(t *testing.T) {
	...
	b, err := dec.ReadByte()
	if err != nil {
		t.Fatal("ReadByte: returned an error", err)
	}
	if b != 'a' {
		t.Fatal("ReadByte: not return correct byte")
	}

	// attempt to decode
	err = dec.Decode(&o)
	if err == nil {
		t.Fatal("expected Decode() to error")
	}
	...
	err = dec.Decode(&o)
	if err != nil {
		t.Fatal("expected Decode() to not error")
	}

	// ensure empty buffer
	b, err = dec.ReadByte()
	if err == nil {
		t.Fatal("ReadByte: should return an error")
	}
	if err != io.EOF {
		t.Fatal("ReadByte: should return eof at end of stream")
	}
	if b != 0 {
		t.Fatal("ReadByte: should return zero byte at end of stream")
	}
}
